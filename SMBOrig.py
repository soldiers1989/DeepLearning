...
class SMBEmb():
  ...
  def _init_graph(self):
    ...
    with tf.name_scope('input') as scope:
      ...
      self.state = [tf.placeholder(tf.float32, [self.args['batch_size'], self.args['emb_size']], name='rnn_state') for _
  ...
  def savemodel(self, step, session): 
    ...
    if step==self.args['total_batch']:
      ...
      inputs = {'x': tf.saved_model.utils.build_tensor_info(self.singleX), \
                'k': tf.saved_model.utils.build_tensor_info(self.keep_prob)}                      
      # y 为最终需要的输出结果tensor
      outputs = {'p': tf.saved_model.utils.build_tensor_info(self.toppred)}
      for jj in range(self.args['layers']):
        inputs['s%d' % jj] = tf.saved_model.utils.build_tensor_info(self.singleState[jj])
        outputs['ps%d' % jj] = tf.saved_model.utils.build_tensor_info(self.singleFState[jj])  
      ...
      # 新增模型
      builder.add_meta_graph_and_variables(sess=session, tags=['org_smb_model'], signature_def_map={'org_smb_model': signature})
      # 保存
      builder.save()
  ...
  def computeMetrics(self, itemindex, totalcnt, nrr, recall10, recall20, recall40, recall100, recall200, recall400):
    ...
    if totalcnt % 20000 == 0 and totalcnt > 0: self.printMetrics(totalcnt, nrr, recall10, recall20, recall40, recall100,
                                                                 recall200, recall400)
    ...
    with tf.Session() as sess:
      ...
      while predata['L'] > 0:
        ...
        for p, r, pr, vid in zip(predata['padding'], nextpredata['restart'], pred, nextpredata['Xid']):
          ...
          totalcnt, nrr, recall10, recall20, recall40, recall100, recall200, recall400 = self.computeMetrics(itemindex, \
  ...
  def infersingleuser(self):
    with tf.Session() as sess:
      ...
      for idx in range(lenlist):
        ...
        state, pred, toppred = sess.run([self.singleFState, self.singlepredyhat, self.toppred], feed_dict=feed_dict)
...
def parse_args(): #--tasks dailytrain --inputpath data/ --modelpath model/
  ...
  parser.add_argument('--tasks', nargs='+', default=['train'],  # ['train', 'dailytrain', 'predvedio', 'preduserset', 'preduser'],
  ...
  parser.add_argument('--ckpt', nargs='?', default='D:\\DeepLearning\\model\\rnn-model-20180911154900-1000',
...
if __name__ == "__main__":
  ...
  if 'predvedio' in tasks or 'preduserset' in tasks or 'preduser' in tasks:
