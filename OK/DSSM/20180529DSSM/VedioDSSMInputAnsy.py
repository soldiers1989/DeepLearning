...
class VedioDSSMInputAnsy(object):
  ...
  def processing_batch(self, lines):
    ...
    for line in lines:
      ...
      label.append(self.parseFixArray(fields[0],1))
#      if float(fields[0])>0.5:
#       label.append(np.array([1.0,0.0]))
#      else:
#       label.append(np.array([0.0,1.0]))

      user_base.append(self.parseFixArray(fields[1],4))
      user_cctype.append(self.parseVarArray(fields[2]))
      user_cclassid1.append(self.parseVarArray(fields[3]))
      user_cclassid2.append(self.parseVarArray(fields[4]))
      user_ctag.append(self.parseVarArray(fields[5], self.vedioattr_size))

      pos_vinfo.append(self.parseFixArray(fields[6],3))
      pos_ctag.append(self.parseVarArray(fields[7], self.vedioattr_size))

      raw_data.append(fields[8])
    ...
    return {'L': len(user_base),
            ...
            'item_vinfo': pos_vinfo,
            'item_ctag': self.varArray2SparseTensorValue( pos_ctag, self.vedioattr_size),
            'raw_data': raw_data }
...
if __name__ == '__main__':
  ...
  parser.add_argument('--dataset', nargs='+', default=['dssmdata', 'dssmdata'],
                      help='Choose a train dataset.')
  parser.add_argument('--testset', nargs='*', default=['dssmdata', 'dssmdata'],
                      help='Choose a test dataset.')
  parser.add_argument('--predset', nargs='*', default=['dssmdata', 'dssmdata'],
  ...
  print(vars(args))
  readdata = VedioDSSMInputAnsy(vars(args))
