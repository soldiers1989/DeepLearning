...
param = {
  ...
  'item_num': 50001,
  'emb_size': 168,  # also used for rnn
  'layers': 2,
  'titlemax_size': 20,
  'articlemax_size': 200,
...
class SMBEmb():
  ...
  def _init_graph(self):
    ...
    with tf.name_scope('loss') as scope:
      ...
      # self.batchNewLogits = tf.matmul(tf.squeeze(self.batchNewOutput, [0]), self.softmax_W[:5000],
      ...
      # 输出 网络输出的batchNewOutput，softmax_W/softmax_b全局
      self.batchNewLogits = tf.matmul(tf.squeeze(self.batchNewOutput, [0])[-1:], self.softmax_W[:8000],
                      transpose_b=True) + self.softmax_b[:8000]

      self.batchNewPred = tf.nn.top_k(self.batchNewLogits, 400).indices
      self.batchNewPred = tf.squeeze(self.batchNewPred)
      ...
      while predata['L'] > 0:
        ...
        for p, r, pr, vid in zip(predata['padding'], nextpredata['restart'], pred, nextpredata['Xid']):
          ...
          totalcnt, nrr, recall10, recall20, recall40, recall100, recall200, recall400 = self.computeMetrics(
            itemindex, totalcnt, nrr, recall10, recall20, recall40, recall100, recall200, recall400)
