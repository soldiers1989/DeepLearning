...
def train_data_generator(dateset):
  while True:
    ...
    yield [np.array(train_data['Q']), np.array(train_data['D'])], np.array(train_data['Y']).transpose()


def validation_data_generator(dateset):
  while True:
    valid_data = readdata.read_testdata_batch(1024)
    yield [np.array(valid_data['Q']), np.array(valid_data['D'])], np.array(valid_data['Y']).transpose()
...
def train3(vocab_size, dateset):
  ...
  model.compile(optimizer=optimizers.Nadam(lr=0.002), loss=paper_loss, metrics=[accuracy, precision, recall, auc])
...
#  callbacks_list = [checkpoint]
  ...
  for it in range(param['outer_epochs']):
    ...
    modelfilepath = param['inputpath'] + "content.model." + ts + ".h5py"
    content_model.save(modelfilepath)
