...
class NeuralFM(BaseEstimator, TransformerMixin):
    def __init__(self, features_M, hidden_factor, layers, loss_type, pretrain_flag, epoch, batch_size, learning_rate, lamda_bilinear,
        ...
        self.hidden_factor = hidden_factor              # 8
        self.layers = layers                            # [32,10]
        self.loss_type = loss_type                      # square_loss
        self.pretrain_flag = pretrain_flag              # 0
        self.features_M = features_M                    # 5382
        self.lamda_bilinear = lamda_bilinear            # 0.0000
        self.epoch = epoch                              # 200
        self.random_seed = random_seed                  # 2016
        self.keep_prob = np.array(keep_prob)            # [0.8,0.5]
        self.no_dropout = np.array([1 for i in range(len(keep_prob))])
                                                        # [1 1]
        self.optimizer_type = optimizer_type            # AdagradOptimizer
        self.learning_rate = learning_rate              # 0.0500
        self.batch_norm = batch_norm                    # 1
        self.verbose = verbose                          # 1
        self.activation_function = activation_function  # relu
        self.early_stop = early_stop                    # 1
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            nonzero_embeddings = tf.nn.embedding_lookup(self.weights['feature_embeddings'], self.train_features)
              #变相的做矩阵乘法，按照ids顺序返回params中的第ids行
              #看训练数据应该是纯dummy
            ...
            if self.verbose > 0:
                print("#params: %d" %total_parameters )  # params: 49067

    def _initialize_weights(self):
        all_weights = dict()
        if self.pretrain_flag > 0: # with pretrain，用FM的权重
            ...
            print('feature_embeddings [features_M: %d, hidden_factor: %d]' % (self.features_M, self.hidden_factor))
            # [features_M: 5382, hidden_factor: 8]

            all_weights['feature_bias'] = tf.Variable(tf.random_uniform([self.features_M, 1], 0.0, 0.0), name='feature_bias')  # features_M * 1
            print('feature_bias [features_M: %d, 1]' % self.features_M)
            # [features_M: 5382, 1]
        ...
        if num_layer > 0:
            glorot = np.sqrt(2.0 / (self.hidden_factor + self.layers[0])) # 哈维尔初始化
            ...
            print('layer_0 [hidden_factor: %d, layers[0]: %d]' % (self.hidden_factor, self.layers[0]))
