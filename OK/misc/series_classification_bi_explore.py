...
np.set_printoptions(threshold=10240)
...
def dynamicRNN(x, seqlen, weights, biases):
    ...
    bioutputs, states = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell,
        x, dtype=tf.float32, sequence_length=seqlen)
        
    outputscat=tf.concat(bioutputs, 2)  # (3, 20, 128) = (batch_size, max_seq_len, n_hidden*2)
    ...
    batch_size = tf.shape(outputscat)[0]
    # 得到每一个序列真正的index
    # tf.range 创建一个数字序列
    ...
    outputs = tf.gather(tf.reshape(outputscat, [-1, n_hidden*2]), index)  # (3, 128)
    # size: (3, 64) 取了60里的[ 8 37 58]

    # 给最后的输出
    return bioutputs, outputscat, outputs, tf.matmul(outputs, weights['out']) + biases['out']

# 这里的pred是logits而不是概率
bioutputs, outputscat, outputs, pred = dynamicRNN(x, seqlen, weights, biases)
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        # 每run一次就会更新一次参数
        bioutputs1, outputscat1, outputs1, pred1, _ = sess.run([bioutputs, outputscat, outputs, pred, optimizer], feed_dict={x: batch_x, y: batch_y,
                                       seqlen: batch_seqlen})
        #print('bioutputs1 %s '%(str(bioutputs1.shape)))
        print('outputscat1 %s '%(str(outputscat1.shape)))
        print('outputscat1 %s '%(str(outputscat1)))
        print('outputs1 %s '%(str(outputs1.shape)))
        print('outputs1 %s '%(str(outputs1)))
        print('pred1 %s '%(str(pred1.shape)))
