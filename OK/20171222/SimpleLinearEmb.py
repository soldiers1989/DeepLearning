...
class SimpleNNEmb(BaseEstimator, TransformerMixin):
  def __init__(self, args, dataset, layers, keep_prob, random_seed=0):
    ...
    self.model_path = args.modelpath + '/' + self.timestamp
  ...
  def _init_graph(self):
    ...
    with self.graph.as_default():  # , tf.device('/cpu:0'):
      ...
      for variable in self.weights.values():
        ...
        print('variable.name: '+variable.name+' variable_parameters: '+str(variable_parameters))
  ...
  def _initialize_weights(self):
      ...
      if self.feature_embedding:
        for idx, item in enumerate(self.embedding_conf):
          ...
          if self.embedding_init_zero:
            all_weights[weight_name] = tf.Variable(tf.zeros([item[2], self.embedding_size], tf.float32),
                                                   name=weight_name)
          else:
    ...
    all_weights['layer_0'] = tf.Variable(
      np.random.normal(loc=0, scale=glorot, size=(emb_dim, self.layers[0])), dtype=np.float32)
    print('layer_0 [input: %d, layers[0]: %d]' % (emb_dim, self.layers[0]))
    ...
    glorot = np.sqrt(2.0 / (self.layers[-1] + 1 + self.dim))
