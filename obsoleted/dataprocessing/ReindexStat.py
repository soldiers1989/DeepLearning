...
class StatData(object):
  ...
  def filestat(self, fname):
    ...
    while cnt > 0:
      with self.idmaplock:
        self.totalline += len(features)
        thisnum = 0
        for item in features:
          for field in item:
            self.totalfield += 1
            if field[0] not in self.idstat:
              self.idstat[field[0]] = 1
            else:
              self.idstat[field[0]] += 1
      cnt, labels, features = tfr.read_sparse_batch()
...
#      self.idmap[k]=self.maxidx
  ...
  def loadidmap(self):
    toload = pickle.load(open(self.statfile, "br"))
    self.idstat = toload['map']
    self.maxidx = toload['maxidx']
    self.idmap={}
    if 'idmap' not in toload:
