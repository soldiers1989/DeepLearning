...
param = {
    ...
    'emb_reduced_size': 32,  # also used for rnn
    'layers': 2,
    'titlemax_size': 20,
    'articlemax_size': 200,
...
class SMBEmb():
    ...
    def _init_graph(self):
        ...
        with tf.device('/cpu:0'):
            with tf.name_scope('embedding') as scope:
                ...
                self.softmax_W = tf.get_variable('softmax_w', [self.args['item_num'], self.args['emb_reduced_size']],
                                                 initializer=initializer)
        ...
        with tf.name_scope('rnn') as scope:
            ...
            self.outputmid, self.final_state = self.stacked_cell(self.xemb, tuple(self.state))

            self.batchNewOutput, _ = tf.nn.dynamic_rnn(self.stacked_cell, self.batchNewXemb,
                                                       sequence_length=self.batchNewXLen, dtype=tf.float32)
            self.batchNewOutputmid = tf.squeeze(self.batchNewOutput, [0])
            
        with tf.name_scope('fc') as scope:
            self.fc1_w0, self.fc1_b0 = TFBCUtils.create_w_b(self.args['emb_size'], self.args['emb_reduced_size'], w_name="fc1_w0", b_name="fc1_b0")
            self.output = tf.nn.relu(tf.matmul(self.outputmid, self.fc1_w0) + self.fc1_b0)
            self.batchNewOutput = tf.nn.relu(tf.matmul(self.batchNewOutputmid, self.fc1_w0) + self.fc1_b0)
        ...
        with tf.name_scope('loss') as scope:
            ...
            self.batchNewLogits = tf.matmul(self.batchNewOutput, self.softmax_W[:5000],
                                            transpose_b=True) + self.softmax_b[:5000]
            self.batchNewYhat = self.final_activation(self.batchNewLogits)

            self.batchNewPred = tf.nn.top_k(self.batchNewYhat[-1], 400).indices
    ...
    def savemodel(self, step, session):
        ...
        inputs = {'x': tf.saved_model.utils.build_tensor_info(self.batchNewX),
                  'xlen': tf.saved_model.utils.build_tensor_info(self.batchNewXLen),
                  'k': tf.saved_model.utils.build_tensor_info(self.keep_prob)}

        # y 为最终需要的输出结果tensor batchtoppred
        outputs = {'p': tf.saved_model.utils.build_tensor_info(self.batchNewPred)}  #

        # 'p_no_sort': tf.saved_model.utils.build_tensor_info(self.batchNewYhat), \
        # 'p_just_rnn': tf.saved_model.utils.build_tensor_info(self.batchNewOutput)
    ...
    def infersingleuserseq(self):
        with tf.Session() as sess:
            ...
            feed_dict = {self.batchNewX: [self.args['predsingleuser']],
                         self.batchNewXLen: [len(self.args['predsingleuser'])], self.keep_prob: 1}
            emb, out, logit, yhat, pred = sess.run(
                [self.batchNewXemb, self.batchNewOutput, self.batchNewLogits, self.batchNewYhat, self.batchNewPred], \
                feed_dict=feed_dict)
            print(str(emb.shape))
            print(str(out.shape))
            print(str(logit.shape))
            print(str(yhat.shape))
