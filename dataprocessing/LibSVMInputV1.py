...
#'''))
    ...
    self.preddata = TxtFilesRandomReader(dim=self.rawdim, files=self.predset, shuffleFile=False, shuffleRecord=False, epochs=1)
  ...
  def loadidmap(self):
    ...
    for item in self.feature_conf:
      validkey = len([k for k in self.idmap.keys() if item[self.CONF_INDEX_ORG_BEGIN]<=k and k<=item[self.CONF_INDEX_ORG_END]])
  ...
  def create_feature_cross_info(self):
    ...
    if self.feature_cross:
      ...
      print(self.cross_raw_feature_map)
  ...
  def processing_batch(self, lines):
    ...
    for line in lines:
      fields = line.strip().split()
      label = int(fields[0])
      features.append(self.processing_feature(fields))
      labels.append(label)

    return {'X':features, 'Y':labels, 'D': len(lines)}
  ...
  def read_preddata_batch(self, size=256):
    ...
    return {'X':features, 'ID':labels, 'D': len(lines)}
  ...
  def print_data(self, line):
    ...
    for item in feaures:
      while fidx<self.feature_num:
        if self.feature_conf[fidx][self.CONF_INDEX_ORG_BEGIN]<=item[0] and item[0]<=self.feature_conf[fidx][self.CONF_INDEX_ORG_END]:
          if item[0] not in self.idmap:
            print(" Name: "+self.feature_conf[fidx][self.CONF_INDEX_NAME]+" Value:"+str(item)+" skipped" )
          else:
            print(" Name: "+self.feature_conf[fidx][self.CONF_INDEX_NAME]+" Value:"+str(item)+" Now index:"+str(self.idmap[item[0]]))
          break
        else:
          fidx+=1
...
if __name__ == '__main__':
  ...
  parser.add_argument('--feature_cross', type=str2bool, default=True,
            help='cross feature.')
  parser.add_argument('--printinfoanddata', type=str2bool, default=False,
